...
for nb in *.ipynb; do
    ...
    if [ -f "$out" ]; then
        ...
        decktape rise "http://127.0.0.1:8888/notebooks/$nb?token=$token" -s 1920x1080 "$out" --chrome-arg=--disable-web-security --chrome-arg=--disable-dev-shm-usage --chrome-path=/usr/bin/chromium
    fi
done
