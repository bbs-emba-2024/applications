...
def plot_ctr_distribution(data, figsize=None, nbins=15):
    plt.figure(figsize=figsize)
    plt.subplot(131)
...
def plot_nn_model(model, show_layer_names=True, show_layer_activations=True, show_shapes=True, **args):
    return keras.utils.plot_model(model, show_shapes=show_shapes,
            show_layer_names=show_layer_names, rankdir='LR',
            show_layer_activations=show_layer_activations, **args)


def plot_training_history(history=None,
        figsize=None, print_final_scores=True, display_loss_curve=True):
    if display_loss_curve:
        plt.figure(figsize=figsize)
