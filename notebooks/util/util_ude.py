...
def simulate_SIR(S0, I0, R0, beta, gamma, tmax, steps_per_day=1):
    ...
    # Wrapper
    dy = lambda y, t: SIR(y, beta, gamma)
    # Solve
...
class NPISIRDerLayer(keras.layers.Layer):
    def __init__(self, beta_pred, gamma_ref=0.1, fixed_gamma=None):
        super(NPISIRDerLayer, self).__init__()
...
def activation_function_plot(figsize=None):
    z = np.linspace(-5, 5)
    y1 = 1 / (1 + np.exp(-z))
    y2 = np.maximum(0, z)
    y3 = z
    ...
    # Sigmoid
    plt.subplot(1, 3, 1)
    plt.plot(z, y1, label=r'sigmoid: $h(z) = \frac{1}{1 + e^{-z}}$')
    plt.legend()
    plt.grid()
    # ReLU
    plt.subplot(1, 3, 2)
    plt.plot(z, y2, label=r'ReLU: $h(z) = \max(0, z)$')
    plt.legend()
    plt.grid()
    # Linear
    plt.subplot(1, 3, 3)
    plt.plot(z, y3, label=r'linear: $h(z) = z$')
    plt.legend()
    plt.grid()
    plt.show()


def load_real_estate_data(fname, test_size=0.25, seed=42, standardize=True):
    ...
    if standardize:
        ...
        res = (data, X_tr, X_ts, y_tr, y_ts, x_scaler, y_scaler)

    return res


def plot_prediction_scatter(y_true, y_pred, figsize=None, display_r2=True, alpha=0.5):
    plt.figure(figsize=figsize)
    plt.scatter(x=y_true, y=y_pred, alpha=alpha)
