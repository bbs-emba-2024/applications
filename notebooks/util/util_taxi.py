...
def plot_series(data, labels=None,
    ...
    if y_cap is not None:
        ymin, ymax = plt.ylim()
        plt.ylim(bottom=np.min(data), top=min(ymax, y_cap))
...
def plot_histogram(data, bins=10, vmin=None, vmax=None, figsize=figsize,
    ...
    # Plot vertical lines, to highlight reference values
    if highlight_values is not None:
        ymin, ymax = plt.ylim()
        plt.vlines(highlight_values, ymin=ymin, ymax=ymax, color='tab:orange')
    # Update limits
    lims = plt.xlim()
    if vmin is not None:
        lims = (vmin, lims[1])
...
def plot_density_estimator_1D(estimator, xr, figsize=figsize, highlight_values=None):
    # Build a new figure
    plt.close('all')
    plt.figure(figsize=figsize)
...
def train_kde(data, bandwidth_range):
    # Scale the data
    # scaler = MinMaxScaler()
    # scaler = StandardScaler()
    scaler = RobustScaler()
    data2 = data.copy()
    data2[:] = scaler.fit_transform(data.values)
    # Calibrate the bandwidth via grid search and cross validation
    params = {'bandwidth': bandwidth_range}
    opt = GridSearchCV(KernelDensity(kernel='gaussian'), params, cv=5)
    opt.fit(data2.values)
    # Return a pipeline
    pipe = Pipeline([('scaler', scaler), ('estimator', opt.best_estimator_)])
    return pipe
